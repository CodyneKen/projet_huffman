...
void find2Lowest(noeud **arbre, int nbElement, int *low1, int *low2) {
    ...
    if (arbre) {
        ...
        if (nbElement == 1) {
            /* tant qu'arbre[i] pas défini (donc pas d'occurrence) */
            while (!arbre[i])
                i++;
...
int main(int argc, char **argv) {
    ...
    /* on calcul le nbre d'occurrences des != caracteres du fichier */
    occurence(f, occ);

    noeud **arbre_huffman = NULL, **alphabet = NULL;
    arbre_huffman = (noeud **) calloc(N_CHAR, sizeof(noeud *));
    ...
    /* on crée les != feuilles de l'arbre à partir du tableau occ */
    for (i = 0; i < N_CHAR; i++) {
        if (occ[i]) {
            taille++;
    ...
    /* On boucle jusqu'a n'avoir plus qu'un seul nœud */
    while (taille > 1) {
    ...
    char *compname = strcat(argv[1], "comp");
