...
void print_man(char **argv) {
    ...
    fprintf(stderr, "void launch_compn(FILE *file, char *fname, char *fout);\n\t");
    fprintf(stderr, "void launch_decomp(FILE *file, char *directory);\n\n");
    fprintf(stderr,
            "DESCRIPTION\n\tLa fonction launch_comp() permet de compresser un fichier en suivant le codage de Huffman\n");
    fprintf(stderr,
            "\tLa fonction launch_compn() permet de compresser plusieurs fichiers dans une meme archive en suivant le codage de Huffman\n");
    fprintf(stderr,
...
void launch_comp(FILE *file, char *fname) {
    ...
    char *compname = (char *) malloc((strlen(fname) + 5) * sizeof(char));
    if (!compname) {
...
void launch_decomp(FILE *file, char *comp_name) {
    FILE *fout = NULL;
    char *file_name = (char *) malloc((strlen(comp_name) + 1) * sizeof(char));
    if (!file_name) {
        fprintf(stderr, "Erreur lors de l'allocation memoire de file_name\n");
        exit(EXIT_FAILURE);
    }
...
void launch_compn(FILE *file, char *fname, char *fout) {
    /* Dans cette fonction a partir d'un fichier on va lancer sa compression.*/
    int i, taille = 0;
    int *occ = NULL;
    noeud **arbre_huffman = NULL, **alphabet = NULL;
...
int main(int argc, char **argv) {
    ...
    char *ext, *fout;
    fout = (char *) malloc((strlen(argv[2]) + 5) * sizeof(char));
    if (!fout) {
        fprintf(stderr, "Erreur lors de l'allocation de fout\n");
        exit(EXIT_FAILURE);
    }
    compression = decompression = 0;
    ...
    strcpy(fout, argv[2]);
    strcat(fout, "comp");
    if (access(fout, F_OK) == 0) /* test si le fichier correspondant a fout existe */
        remove(fout); /* supprime le fichier s'il existe deja. Necessaire pour pouvoir ajouter par la suite */
    if (compression) {
        for (i = 2; i < argc; i++) {
            fin = fopen(argv[i], "r");
            if (!fin) {
