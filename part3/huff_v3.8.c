...
int main (int argc, char** argv){
  ...
  /* on trouve le noeud qui contient désormais la racine */
  i=0;
  while(!arbre_huffman[i])
    i++;

  display_noeud(arbre_huffman[i]->gauche);
  display_noeud(arbre_huffman[i]->droit);

/* on sait que i est non-vide, pour tester: */
  creer_code(arbre_huffman[i]);
  affichage_code(alphabet[i]->nb_bit, alphabet[i]->enc);
